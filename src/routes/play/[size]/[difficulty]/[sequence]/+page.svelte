<script lang="ts">
	...
	let catalog = $state<CatalogSlot[]>(data.catalog);
	...
	let phase = $state<'play' | 'won' | 'lost'>(data.progress?.solved ? 'won' : 'play');
	let recorded = $state(Boolean(data.progress?.solved));
	...
	let savedStars = $state<number | null>(data.progress?.solved ? data.progress.stars : null);
	let resetSizes = $state<BoardSize[]>([data.puzzle.size ?? 7]);
	let resetLevels = $state<Difficulty[]>([data.puzzle.difficulty ?? 'medium']);

	let stars = $derived(
		savedStars ?? starsFor(puzzle.swapLimit - swapsLeft, phase === 'won', puzzle.swapLimit, puzzle.minSwaps)
	);
	...
	async function record(finalBoard: number[][], used: number) {
		...
		catalog = catalog.map((slot) =>
			slot.id === puzzle.id
				? { ...slot, attempted: true, solved: result.solved, stars: result.stars }
				: slot
		);
		if (result.solved) savedStars = result.stars;
	}
	...
	function swap(r1: number, c1: number, r2: number, c2: number) {
		if (phase !== 'play' || busy || savedStars !== null) return;
	...
	async function undo() {
		...
		const wasRecorded = recorded;
		board = previous.board.map((row) => row.slice());
		swapsLeft = previous.swapsLeft;
		phase = previous.phase;
		recorded = previous.recorded;
		savedStars = null;
		revealed = false;
		selected = null;
		if (wasRecorded && !previous.recorded) {
	...
	function applyOpened(payload: {
		...
		stats = payload.stats;
		sizeChoice = payload.puzzle.size;
		difficultyChoice = payload.puzzle.difficulty;
		savedStars = payload.progress?.solved ? payload.progress.stars : null;
		history = [];
		selected = null;
		revealed = false;
		dialog = null;
	...
	async function openSet(size = sizeChoice, difficulty = difficultyChoice, sequence?: number) {
		...
		try {
			const response = await fetch('/api/puzzles', {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({ size, difficulty, sequence })
			});
			if (!response.ok) return;
			applyOpened(await response.json());
		} finally {
			busy = false;
		}
	}
	...
	function nextOpen() {
		const current = puzzle.sequence ?? 0;
		const target =
			catalog.find((slot) => slot.sequence > current && !slot.solved) ?? catalog.find((slot) => !slot.solved);
		if (target) openSet(sizeChoice, difficultyChoice, target.sequence);
	}
		...
		try {
			const response = await fetch('/api/progress', {
				...
				body: JSON.stringify({
					...
					catalogSize: sizeChoice,
					catalogDifficulty: difficultyChoice
				})
			});
			if (!response.ok) return;
			const payload = await response.json();
			stats = payload.stats;
			catalog = payload.catalog;
			const currentCleared = resetSizes.includes(sizeChoice) && resetLevels.includes(difficultyChoice);
			if (currentCleared) {
				savedStars = null;
				recorded = false;
				phase = 'play';
...
<svelte:head>
	<title>Myable {puzzle.sequence}</title>
</svelte:head>
...
{#if dialog === 'reset'}
	...
	<div class="sheet" role="dialog" aria-labelledby="reset-title">
		...
		<p>Clear the ticks and stars for any mix of sizes and difficulties. The puzzles stay. Only the record of how you did is removed.</p>
		<p class="choice">
			<span>Size</span>
			{#each SIZES as size}
...
<style>
	...
	.slot.on {
		outline: 2px solid oklch(0.32 0.04 55);
		outline-offset: 1px;
	}
	...
	.expand {
		...
		line-height: 1;
		cursor: pointer;
		box-shadow: 0 0 0 1px oklch(0.78 0.01 260);
	}
	...
	.stage.focus {
		...
		background: oklch(0.84 0.004 260);
	}

	.stage.focus .board {
		width: min(100%, calc(100svh - 4.5rem));
		max-width: min(100%, calc(100svh - 4.5rem));
	}
